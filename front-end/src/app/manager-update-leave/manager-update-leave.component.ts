...
@Component({
  selector: 'app-manager-update-leave',
  templateUrl: './manager-update-leave.component.html',
  styleUrls: ['./manager-update-leave.component.css']
})
export class ManagerUpdateLeaveComponent implements OnInit {
    ...
    this.updateLeaveService.updateApply(this.myApply).subscribe(
      (data) => {
        if (data['success'] === true) {
          alert('success');
          this.router.navigateByUrl('manager/search-leave');
        } else {
          alert('error');
        }
      }
  ...
  ngOnInit() {
    ...
    this.myApply.recordId = this.LSData.getObject('updateManagerLeaveId');
    this.getApply();
  }
}
