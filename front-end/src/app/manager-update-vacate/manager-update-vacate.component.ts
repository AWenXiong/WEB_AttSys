...
@Component({
  selector: 'app-manager-update-vacate',
  templateUrl: './manager-update-vacate.component.html',
  styleUrls: ['./manager-update-vacate.component.css']
})
export class ManagerUpdateVacateComponent implements OnInit {
    ...
    this.updataVacateService.updateApply(this.myApply).subscribe(
      (data) => {
        if (data['success'] === true) {
          alert('success');
          this.router.navigateByUrl('manager/search-vacate');
        } else {
          alert('error');
        }
      }
  ...
  ngOnInit() {
    ...
    this.myApply.recordId = this.LSData.getObject('updateManagerVacateId');
    this.getApply();
  }
